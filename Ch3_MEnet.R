...
Q2 <- function(q2,knot_values){
  
  Q2_a <- diag(as.vector(q2*(abs(knot_values)**(q2-3))*knot_values))
  Q2_a[1,1] <- 0 
  return(Q2_a)
}
...
beta_estimates_different_lambda <- function(x,y,alpha,q2,it)
{
...
cross_validation <- function(num_folds,x,y,alpha,q2,it){
  ...
  for(i in 1:k){
    train_x <- x[folds$subsets[folds$which != i], ]
    test_x <- x[folds$subsets[folds$which == i], ]
    train_y <- y[folds$subsets[folds$which != i], ]
    test_y <- y[folds$subsets[folds$which == i], ]
...
choose_optimal <- function(num_folds,x,y,alpha,q2,it,auc_or_prediction_error){
...
optimal_optimal <- function(num_folds,x,y,alpha,q2,it,auc_or_prediction_error){
  
  choose_optimal_1 <- choose_optimal(num_folds,x,y,alpha,q2,it,auc_or_prediction_error) 
...
optimal_with_thresholds <- function(num_folds,x,y,alpha,q2,it,auc_or_prediction_error,quantile_sequence){
  
  optimal <- optimal_optimal(num_folds,x,y,alpha,q2,it,auc_or_prediction_error)
  lambda <- optimal[[1]][[1]]
...
optimal_with_thresholds_performance <- function(num_folds,x,y,alpha,q2,it,
                                                auc_or_prediction_error,quantile_sequence){
...
alpha0_q1 <- mclapply(empty_vec, simulation2, num_folds = num_folds, x = x, y = y,
                                alpha = alpha, q2 = q2, it = 1000,
                                auc_or_prediction_error = auc_or_pred,
                                quantile_sequence = quantile_sequence, mc.cores = 60)
